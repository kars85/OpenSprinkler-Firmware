...
**Producer status:** the current OpenSprinkler-Weather legacy converter emits top-level `restricted=1` when its rain restriction fires, alongside `scale=0`. The producer contract and `test/firmware-contract.spec.ts` pin that additive behavior as **FR-P0.4**.
...
- **Watering suppression — PASS (requires `prog.use_weather`).** Scheduling forces `wl = 0` when `wt_restricted > 0` for weather-enabled programs (`main.cpp:908`); `wl = 0` scales `water_time` to `0`, so no station is queued (`main.cpp:938-953`).
- **Status surface — PASS.** `/jc` emits `"wtrestr":wt_restricted` (`opensprinkler_server.cpp:1281-1290`).
- **Skip notification — PASS.** Scheduling tracks `prog_queued` separately for each matched program (`main.cpp:924`, set at `:953`) so an earlier program cannot hide a later skip. A fully skipped weather-enabled program passes `wt_restricted`; a program that does not use weather passes `0` (`main.cpp:960-967`). `notifier.cpp` then publishes MQTT `{"state":"skipped","wtrestr":<bval>}` and appends *"due to weather restriction."* to IFTTT/email only when `bval > 0` (`notifier.cpp:378-392`).

**Edge cases (verified):** `restricted` is parsed even when `errCode != 0` (so a stale value can't persist — it is reset to `0` when absent); the weather-success-timeout reset zeroes `wt_restricted` (along with `wl→100`) for non-manual methods (`main.cpp:1243-1254`), so a restriction self-clears if the service stops responding (fail-open).
...
- Parsed entries are stored in `md_scales` and counted in `md_N` (`weather.cpp:278-294`).
- Interval programs can consume those historical values during scheduling (`main.cpp:912-918`).
- `/jc` exposes the active array as `wls` (`opensprinkler_server.cpp:1297-1303`).

Normal OpenSprinkler-Weather decisions do not generate a top-level `scales` key, but the legacy converter preserves that reserved field when a caller supplies it. Without it, `md_N` stays `0` and the consumer branch is unreachable (`main.cpp:912` requires `mda == 100 && prog.type == PROGRAM_TYPE_INTERVAL && md_N > 0`; timeout clearing is at `main.cpp:1254`).
...
| `restricted` | optional-parsed | Drives `wt_restricted`, `wtrestr`, restriction-aware skips, and restriction-specific notifications when present (`weather.cpp:82-86`, `main.cpp:908-967`, `opensprinkler_server.cpp:1281-1290`, `notifier.cpp:375-407`). |
...
| `rawData` | optional-parsed | Stored opaquely in `wt_rawData` and surfaced through `/jc` as `wtdata`; size constrained by `TMP_BUFFER_SIZE` (`weather.cpp:137-139`, `opensprinkler_server.cpp:1281-1289`). |
| `scales` | dormant | Optional 14-entry multiday scale array for interval programs; still parsed and exposed, but only if the producer sends it (`weather.h:35`, `weather.cpp:141-147`, `weather.cpp:278-294`, `main.cpp:912-918`). |
| `rawData.*` | producer-owned | Nested structure is opaque to firmware. The producer may evolve this blob as long as the top-level `rawData` size contract is preserved (`weather.cpp:137-139`, `opensprinkler_server.cpp:1281-1289`). |

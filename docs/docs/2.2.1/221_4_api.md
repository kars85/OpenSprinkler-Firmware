...
* **Authentication** (`pw`): Protected API calls require the device password via the `pw` parameter. The value must be provided as a lowercase MD5 hash. The OpenSprinkler app and web interfaces handle this hashing automatically. If you are making direct API calls, you must hash your plaintext password before sending the request. You can use any standard [MD5 generation tool](https://www.md5hashgenerator.com/) for this. (`/su` is the separate unauthenticated firmware-served UI-injection view.)
* **`/jo` and `/ja` authentication bootstrap**: A missing or incorrect password returns HTTP 200 with only `{"fwv":221}` on these two endpoints. That version-only object is not an authenticated success response; clients must require the normal full response before enabling controls. Other protected endpoints return `result=2` when authentication fails.
* **Modern kars85 App support contract (`2214 + kars85`)**: `2214` is the App's combined comparison `fwv*10+fwm`, not a separate wire field. The modern App preflights `/jo`; numeric `fwv >= 221` selects lowercase-MD5 authentication when password checking is enabled, while the version-only response means authentication is still required or failed. A full options response must report `fwv=221`, `fwm>=4`, and an `fwf` value beginning with `kars85.` to meet the supported floor. This consumer policy does not require or authorize a fork-only `fwv` bump.
* **Fork capability versioning**: Keep `OS_FW_VERSION`/`fwv` aligned with the upstream persisted-storage epoch. `options_setup()` factory-resets when the stored `fwv` differs from the compiled `OS_FW_VERSION`, whereas `OS_FW_MINOR` is not part of that reset comparison. Additive fork capabilities should use validated field presence and, where needed, an upstream-compatible `fwm` boundary; `fwf` identifies the fork without changing persisted version state.
...
| `fwf`             | Fork build tag (e.g., `kars85.1`), identifying the fork build when `fwv`/`fwm` match upstream. **RO**, *fork-only — absent on official firmware*. See `docs/fork-versioning.md`.|

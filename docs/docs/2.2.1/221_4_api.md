...
* **Authentication** (`pw`): Controller data and change endpoints require the device password via `pw`; firmware 2.2.1 expects a lowercase MD5 hash. The OpenSprinkler app and web interfaces handle this hashing. Exceptions are load-bearing: `/su` is an unauthenticated HTML recovery/settings page, `/db` is unauthenticated diagnostics, and a failed password on `/jo` or `/ja` returns HTTP 200 with only `{"fwv":221}` instead of `{"result":2}`. That `fwv`-only response is an authentication-bootstrap shape, not a successful options/all response.
...
* **JSON endpoint return values** include shapes such as `{"fwv":221}` and `{"result":1}`. HTML pages such as `/su` are documented separately. Below is the list of result/error codes:
...
|`lswc` | Timestamp (epoch seconds) of the last **successful** weather response (`errCode == 0`). `0` means no successful response yet.|
...
|`wtdata`| Opaque cached `rawData` relayed from the weather server. Firmware does not parse it to compute watering percentage.|
...
|`wls`  | Active parsed multi-day scale array, maximum 14 entries. It is normally empty because current Weather decisions do not emit `scales`.|
...
With a valid password, `/jo` returns the full option set below (including `wl`). With a missing or invalid password, it deliberately returns HTTP 200 with only `{"fwv":221}` so clients can perform version/authentication bootstrap.

<u>Note</u>: options marked **RO** are read-only and not modifiable via the `/co` command

|Variable |Explanation|
|:--------|:----------|
| `fwv`             | Firmware compatibility/storage epoch (e.g., `221` → v2.2.1). A changed value can trigger a settings reset. **RO**|
| `fwm`             | Reset-free minor/capability revision. It does not participate in settings-reset detection. **RO**|
| `fwf`             | Fork provenance/build tag (e.g., `kars85.3`). **RO**, *fork-only — absent on official firmware*. Use it with an `fwm` floor and field presence; see `docs/fork-versioning.md`.|
...
### 20. Script URL Recovery `/su` and Change `/cu`

#### View recovery/settings page `/su`

**Usage**: <code>http://os-ip/su</code>

Returns an HTML form showing the current UI (`jsp`) and Weather (`wsp`) service URLs. This page intentionally requires no password so a broken UI source can be recovered. Viewing `/su` never persists settings; the form submits the entered password and values to authenticated `/cu`.

#### Persist script URLs `/cu`
...
Only nonempty supplied values are changed; omitting `jsp`/`wsp` or submitting it empty leaves that stored value unchanged.
...
The aggregate shape below requires valid authentication. With a missing or invalid password, `/ja` deliberately returns HTTP 200 with only `{"fwv":221}`.
